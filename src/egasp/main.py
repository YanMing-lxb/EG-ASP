'''
 ...
 -----------------------------------------------------------------------
Author       : 焱铭
Date         : 2025-04-21 19:33:36 +0800
LastEditTime : 2025-04-21 23:39:53 +0800
Github       : https://github.com/YanMing-lxb/
FilePath     : /EGWaterProps/src/main.py
Description  : 
 -----------------------------------------------------------------------
'''

from data import eg_rho, eg_cp, eg_k, eg_mu, eg_fb
...
def get_eg_props(temp: float, conc: float, data_matrix: list, 
                temp_range: tuple = (-35, 125), conc_range: tuple = (0.1, 0.9),
                temp_step: int = 5, conc_step: float = 0.1) -> float | None:
    ...
    if t_lower == t_upper and c_lower == c_upper:
        ...
        return interpolate_linear(c_lower, v11, c_upper, v12, conc)
    elif c_lower == c_upper:
        return interpolate_linear(t_lower, v11, t_upper, v21, temp)
    else:
        v1 = interpolate_linear(c_lower, v11, c_upper, v12, conc)
        v2 = interpolate_linear(c_lower, v21, c_upper, v22, conc)
        return interpolate_linear(t_lower, v1, t_upper, v2, temp)


def get_eg_fb_props(data: list, query: float, query_type: str = 'volume') -> tuple[float, float, float, float] | None:
    """根据质量浓度或体积浓度查询乙二醇溶液的属性

    Parameters
    ----------
    data : list
        数据集，每个元素为 (mass_conc, volume_conc, freezing, boiling)
    query : float
        查询的浓度值
    ...
    if query_type not in ('mass', 'volume'):
        raise ValueError("query_type必须为'mass'或'volume'")
    ...
    for i in range(len(sorted_data) - 1):
        ...
        if current_val <= query <= next_val:
            ...
            if query_type == 'volume':
                mass = interpolate_linear(v1, m1, v2, m2, query)
                volume = query
                freezing = interpolate_linear(v1, f1, v2, f2, query)
                boiling = interpolate_linear(v1, b1, v2, b2, query)
            else:
                volume = interpolate_linear(m1, v1, m2, v2, query)
                mass = query
                freezing = interpolate_linear(m1, f1, m2, f2, query)
                boiling = interpolate_linear(m1, b1, m2, b2, query)
...
mass_volume, volume_volume, freezing_volume, boiling_volume = get_eg_fb_props(eg_fb, volume_query, query_type='volume')
mass_mass, volume_mass, freezing_mass, boiling_mass = get_eg_fb_props(eg_fb, mass_query, query_type='mass')

print(f"体积浓度 {volume_query} 对应的质量浓度: {mass_volume:.2f}, 冰点 {freezing_volume:.2f}℃, 沸点 {boiling_volume:.2f}℃")
print(f"质量浓度 {mass_query} 对应的体积浓度: {volume_mass:.2f}, 冰点 {freezing_mass:.2f}℃, 沸点 {boiling_mass:.2f}℃")